...
class MeshDelegateHandler(
    ...
    private val onSnagReceived: (Snag) -> Unit = {},
    private val onSnagProgressReceived: (String) -> Unit = {},
    private val onSnagResolvedReceived: (String) -> Unit = {},
    private val onSnagDeletedReceived: (String) -> Unit = {},
    private val onBulletinReceived: (ParsedBulletin) -> Unit = {}
    ...
    override fun didReceiveMessage(message: BitchatMessage) {
        coroutineScope.launch {
            ...
            // Snag detection — new snag, progress update, resolve, delete
            if (message.content.startsWith("[SNAG:")) {
                val parsed = parseSnagMessage(message.content, message.sender)
                if (parsed != null) {
                    onSnagReceived(parsed)
                }
            }
            if (message.content.startsWith("[SNAG_PROGRESS:")) {
                val id = message.content.removePrefix("[SNAG_PROGRESS:").removeSuffix("]")
                if (id.isNotBlank()) onSnagProgressReceived(id)
            }
            if (message.content.startsWith("[SITE_PIN_RESOLVED:")) {
                val id = message.content.removePrefix("[SITE_PIN_RESOLVED:").removeSuffix("]")
                if (id.isNotBlank()) onSnagResolvedReceived(id)
            }
            if (message.content.startsWith("[SITE_PIN_DELETED:")) {
                val id = message.content.removePrefix("[SITE_PIN_DELETED:").removeSuffix("]")
                if (id.isNotBlank()) onSnagDeletedReceived(id)
            }

            // Bulletin detection
            if (message.content.startsWith("[BULLETIN:")) {
                val parsed = BulletinMessageParser.parse(message.content)
                if (parsed != null) onBulletinReceived(parsed)
            }
            // [BULLETIN_ACK:] messages are informational — no local action needed
