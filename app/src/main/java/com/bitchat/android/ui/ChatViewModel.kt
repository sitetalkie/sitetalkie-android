...
class ChatViewModel(
    ...
    val snagStore = SnagStore(application.applicationContext)
    ...
    private val meshDelegateHandler = MeshDelegateHandler(
        ...
        onSnagReceived = { snag ->
            snagStore.addFromMesh(snag)
            android.util.Log.d("ChatViewModel", "Received snag from mesh: ${snag.title} by ${snag.createdBy}")
        },
        onSnagProgressReceived = { id ->
            snagStore.updateStatus(id, SnagStatus.IN_PROGRESS)
        },
        onSnagResolvedReceived = { id ->
            snagStore.updateStatus(id, SnagStatus.RESOLVED)
        },
        onSnagDeletedReceived = { id ->
            snagStore.deleteSnag(id)
        },
        onBulletinReceived = { parsed ->
            BulletinStore.addFromMesh(parsed)
            android.util.Log.d("ChatViewModel", "Received bulletin from mesh: ${parsed.title}")
        }
    )
    ...
    // Emergency Handbook deep link — set scenarioId to show protocol screen
    private val _handbookScenarioId = MutableStateFlow<String?>(null)
    val handbookScenarioId: StateFlow<String?> = _handbookScenarioId.asStateFlow()

    fun openHandbookScenario(scenarioId: String) {
        _handbookScenarioId.value = scenarioId
    }

    fun closeHandbookScenario() {
        _handbookScenarioId.value = null
    }
    ...
    init {
        ...
        val savedChannel = application.getSharedPreferences("sitetalkie_prefs", android.content.Context.MODE_PRIVATE)
            .getString("sitetalkie.selectedChannel", null)
        if (savedChannel != null) {
            channelManager.switchToChannel(savedChannel)
        }
    }
    ...
    private fun loadAndInitialize() {
        ...
        // Load data — ensure default channels are always joined
        val (savedChannels, protectedChannels) = channelManager.loadChannelData()
        val joinedChannels = savedChannels + setOf("general", "defects", "deliveries")
    ...
    fun switchToChannel(channel: String?) {
        ...
        getApplication<Application>().getSharedPreferences("sitetalkie_prefs", android.content.Context.MODE_PRIVATE)
            .edit().putString("sitetalkie.selectedChannel", channel).apply()
    }
        ...
        if (selectedPeer != null) {
            ...
            if (selectedLocationChannel is com.bitchat.android.geohash.ChannelID.Location) {
                ...
                if (currentChannelValue != null) {
                    ...
                    // Prepend [CH:tag] for wire format (iOS compatible)
                    val wireContent = "[CH:$currentChannelValue] $content"
