...
import com.bitchat.android.ui.BitChatScreen
import com.bitchat.android.ui.isBitChatMode
import com.bitchat.android.ui.setBitChatMode
...
class MainActivity : OrientationAwareActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        com.bitchat.android.ui.TranslationService.init(applicationContext)
        ...
        when (onboardingState) {
            ...
            OnboardingState.COMPLETE -> {
                ...
                when (appScreen) {
                    ...
                    "main" -> {
                        var bitchatMode by remember { mutableStateOf(isBitChatMode(context)) }
                        if (bitchatMode) {
                            BitChatScreen(
                                viewModel = chatViewModel,
                                onReturnToSiteTalkie = {
                                    setBitChatMode(context, false)
                                    bitchatMode = false
                                }
                            )
                        } else {
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                handleDashboardAuthIntent(intent)
                
                // Small delay to ensure mesh service is fully initialized
                delay(500)

                // Sync site data and bulletins from Supabase
                launch(kotlinx.coroutines.Dispatchers.IO) {
                    try { com.bitchat.android.ui.SiteDataSyncService.sync(applicationContext) } catch (_: Exception) {}
                    try { com.bitchat.android.ui.BulletinSyncService.sync(applicationContext) } catch (_: Exception) {}
                    kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
                        com.bitchat.android.ui.SiteDataStore.load(applicationContext)
                        com.bitchat.android.ui.BulletinStore.load(applicationContext)
                    }
                }
    ...
    private fun handleDashboardAuthIntent(intent: Intent) {
        val uri = intent.data ?: return
        if (uri.scheme != "sitetalkie" || uri.host != "auth") return

        val sessionId = uri.getQueryParameter("session") ?: return
        val challenge = uri.getQueryParameter("challenge") ?: return

        Log.d("MainActivity", "Dashboard auth deep link: session=$sessionId")
        com.bitchat.android.ui.DashboardAuthService.setPendingAuth(sessionId, challenge)
    }
