    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun onMessageReceived(message: BitchatMessage) {
                // Parse [CH:tag] prefix from broadcast messages for channel routing
                val routed = if (!message.isPrivate && message.channel == null && message.content.startsWith("[CH:")) {
                    val closeBracket = message.content.indexOf(']')
                    if (closeBracket > 4) {
                        val channelTag = message.content.substring(4, closeBracket)
                        val strippedContent = message.content.substring(closeBracket + 1).trimStart()
                        message.copy(content = strippedContent, channel = channelTag)
                    } else message
                } else message
                ...
                try {
                    when {
                        routed.isPrivate -> {
                            val peer = routed.senderPeerID ?: ""
                            if (peer.isNotEmpty()) com.bitchat.android.services.AppStateStore.addPrivateMessage(peer, routed)
                        }
                        routed.channel != null -> {
                            com.bitchat.android.services.AppStateStore.addChannelMessage(routed.channel!!, routed)
                        }
                        else -> {
                            com.bitchat.android.services.AppStateStore.addPublicMessage(routed)
                        }
                    }
                } catch (_: Exception) { }
                // And forward to UI delegate if attached
                delegate?.didReceiveMessage(routed)

                // If no UI delegate attached (app closed), show DM notification via service manager
                if (delegate == null && routed.isPrivate) {
                    try {
                        val senderPeerID = routed.senderPeerID
                        if (senderPeerID != null) {
                            val nick = try { peerManager.getPeerNickname(senderPeerID) } catch (_: Exception) { null } ?: senderPeerID
                            val preview = com.bitchat.android.ui.NotificationTextUtils.buildPrivateMessagePreview(routed)
                ...
                if (delegate == null && routed.content.startsWith("[SITE_ALERT:")) {
                    try {
                        val parsed = com.bitchat.android.ui.parseSiteAlert(routed.content)
                        if (parsed != null) {
                            val alertWithSender = parsed.copy(senderName = routed.sender)
